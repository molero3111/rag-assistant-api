...
class SlidingWindowHistory(BaseChatMessageHistory, BaseModel):
...
def get_by_session_id(session_id: str) -> BaseChatMessageHistory:
    if session_id not in store:
        store[session_id] = SlidingWindowHistory()
    return store[session_id]

class Command(BaseCommand):
    help = "Demonstrate the sliding window memory strategy for VAE publication chat (RunnableWithMessageHistory)."
    ...
    def handle(self, *args, **options):
        ...
        session_id = "sliding_window_demo"
        for user_input in turns:
            result = chain_with_history.invoke(
                {"question": user_input},
                config={"configurable": {"session_id": session_id}}
            )
            # Get all messages from history (only last 3 kept)
            ...
            self.stdout.write(f"Total tokens in sliding window (last {store[session_id].window_size} messages): {total_tokens}")
